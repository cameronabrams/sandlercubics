...
class TestCubicEOS(TestCase):
    ...
    def test_ideal_gas_solve_Tv(self):
        ...
        eos = IdealGasEOS(pressure_unit="pa", T=300.0, P=P_as_input).set_compound('methane')
        v_start = eos.v[0]
        eos.P = None
    ...
    def test_ideal_gas_solve_Pv(self):
        T_as_input = 300.0  # K
        eos = IdealGasEOS(pressure_unit="pa", T=T_as_input, P=101325.0).set_compound('methane')
        v_start = eos.v[0]
        eos.T = None
    ...
    def test_peng_robinson_solve(self):
        eos = PengRobinsonEOS(pressure_unit="bar", T=300.0, P=1.01325).set_compound('ethane')
        z = eos.Z[0]
    ...
    def test_peng_robinson_water_saturation(self):
        eos = PengRobinsonEOS(pressure_unit="bar", logiter=True, T=373.15).set_compound('water')
        Pvap = eos.Pvap
    ...
    def test_soave_redlich_kwong_solve(self):
        eos = SoaveRedlichKwongEOS(pressure_unit="bar", T=300.0, P=1.01325).set_compound('methane')
    ...
    def test_soave_redlich_kwong_water_saturation(self):
        eos = SoaveRedlichKwongEOS(T=373.15, pressure_unit="bar", logiter=True).set_compound('water')
    ...
    def test_peng_robinson_solve_Tv(self):
        P_as_input = 10.0
        eos = PengRobinsonEOS(pressure_unit="bar", T=300.0, P=P_as_input).set_compound('methane')
        v_start = eos.v[0]
        eos.P = None
        eos.solve(T=eos.T, v=v_start)
        P_as_dependent = eos.P
    ...
    def test_peng_robinson_solve_Pv(self):
        T_as_input = 350.0
        eos = PengRobinsonEOS(pressure_unit="bar", T=T_as_input, P=15.0).set_compound('ethane')
        v_start = eos.v[0]
        eos.T = None
        eos.solve(P=eos.P, v=v_start)
        T_as_dependent = eos.T
    ...
    def test_peng_robinson_solve_Th(self):
        P_as_input = 11.0
        eos = PengRobinsonEOS(pressure_unit='bar', T=397, P=P_as_input).set_compound('ethane')
        h_sav = eos.h[0]
        eos.P = None
    ...
    def test_peng_robinson_solve_Ts(self):
        P_as_input = 9.0
        eos = PengRobinsonEOS(pressure_unit='bar', T=350, P=P_as_input).set_compound('ethane')
        s_sav = eos.s[0]
        eos.P = None
    ...
    def test_vdw_solve(self):
        eos = VanDerWaalsEOS(pressure_unit='bar', T=400, P=8.0).set_compound('ethane')
        self.assertAlmostEqual(eos.v[0], 0.00405312, places=6)

    def test_peng_robinson_ethane_pvap(self):
        eos = PengRobinsonEOS(pressure_unit='bar', logiter=True, T=270).set_compound('ethane')
        Pvap = eos.Pvap
        self.assertAlmostEqual(Pvap, 22.21845, places=2)  # Expect ~1 atm in bar

    def test_peng_robinson_get_compound(self):
        """
        Tests that .set_compound() works with Compound object
        """
        methane = get_database().get_compound('methane')
        Tc = methane.Tc
        eos = PengRobinsonEOS(pressure_unit='bar', logiter=True, T=300.0, P=1.0).set_compound(methane)
        self.assertEqual(eos.Tc, Tc)
